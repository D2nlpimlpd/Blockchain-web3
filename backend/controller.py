...
def simulate_step(state, stable_contract, use_onchain=False):
    """执行一步模拟。如果 use_onchain=True，则真实更新至区块链。"""
    new_state = compute_new_state(state, step=1)
    ...
    if use_onchain:
        ...
        print(f"🧮 [本地模式] 已模拟更新 price = {price_onchain / 1e18:.4f} USD")
